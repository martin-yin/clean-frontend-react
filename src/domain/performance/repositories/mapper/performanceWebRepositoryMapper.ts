...
export function performanceWebRepositoryMapper() {
  const mapFromPerformancPageListModel = (param: PerformanceListEntity): PerformanceListModel => {
    return param.map(item => toUpperCaseData(item))
  }
  const mapFromPerformanceStackModel = (param: PerformanceStackEntity) => {
    return [
      {
        title: '',
  ...
  const mapFromPerformancQuotaModel = (param: PerformanceQuotaEntity): PerformanceQuotaModel => {
    return toUpperCaseData(param)
  }
  const mapFromPerformanceStageTimeListModel = (param: PerformanceStageTimeListEntity) => {
    const pv = []
    const timeConsumes = []
    param.map(item => {
      pv.push({
  ...
  return {
    mapFromPerformancPageListModel,
    mapFromPerformanceStackModel,
    mapFromPerformancQuotaModel,
    mapFromPerformanceStageTimeListModel
  }
}
