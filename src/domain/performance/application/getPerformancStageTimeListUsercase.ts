...
@injectable()
export class GetPerformanceStageTimeListUseCase implements UseCase<FilterHeaderParams, any> {
  constructor(
  ...
  async execute(params: FilterHeaderParams): Promise<any> {
    const { data, code, msg } = await this.performanceRepository.getPerformanceStageTimeList(params)
    if (code === 200) {
      return this.performanceRepositoryMapper.mapFromPerformanceStageTimeListModel(data)
    } else {
