...
@injectable()
export class GetPerformancQuotaUseCase implements UseCase<FilterHeaderParams, any> {
  constructor(
  ...
  async execute(params: FilterHeaderParams): Promise<any> {
    const { data, code, msg } = await this.performanceRepository.getPerformancQuota(params)
    if (code === 200) {
      return this.performanceRepositoryMapper.mapFromPerformancQuotaModel(data)
    } else {
